...
| Klenke 5.34–5.36 | Poisson-process characterization and constructions | `Probability/Process/Poisson/Basic.lean`; `IntervalAxioms.lean`; `IntervalAxiomsConverse.lean`; `Constructions.lean` | Exact P1–P5 interval-count predicate and the forward implication from a Poisson process implemented. For the converse, interval-mean linearity, the Markov multiple-jump bound, and the upgrade of P5 from `limsup` to a right-hand limit are implemented; identification of all interval laws remains. Deterministic construction cores include arrival/count inversion and right continuity |
