...
| Klenke 5.34 | Empirical CDF and Glivenko–Cantelli | `Probability/EmpiricalProcess/CDF.lean`; `StrongLaw.lean`; `GlivenkoCantelli.lean` | Finite-sample API, pointwise a.s. convergence on one common event for every real threshold, and the uniform a.s. theorem for continuous population CDFs implemented; the general atomic-law uniform theorem remains |
