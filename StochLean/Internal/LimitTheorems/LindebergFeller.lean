...
public import StochLean.Internal.LimitTheorems.CharacteristicFunctionBounds
...
- `lindebergFellerRaw`: the internal theorem consumed by StochLean's public API
...
open StochLean.Internal.LimitTheorems
...
theorem lindebergFellerRaw
    ...
    Tendsto μs atTop
      (𝓝 (⟨gaussianReal (0 : ℝ) (1 : NNReal), inferInstance⟩ : ProbabilityMeasure ℝ)) := by
  intro μs
  ...
  have hconv : ∀ t, Tendsto (fun n => charFun (↑(μs n) : Measure ℝ) t) atTop
    ...
    intro t
    simp_rw [charFun_standardGaussian]
    exact charfun_lindeberg_pointwise hk hs hm hindep hmean hL2 hvar_sum hLindeberg t
  exact ProbabilityMeasure.tendsto_of_tendsto_charFun hconv
