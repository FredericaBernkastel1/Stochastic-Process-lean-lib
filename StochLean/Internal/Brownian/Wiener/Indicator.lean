...
MathFin/Foundations/WienerIntegralIndicator.lean (Apache-2.0).
The proof is internalized here with no external package dependency. -/

public import StochLean.Internal.Brownian.Wiener.Gaussian

/-!
# The Wiener integral of a step indicator is the increment

The Wiener isometry `integral B hB T : L²([0,T]) →L[ℝ] L²(μ)` is built as
`(incrementAssembly).extendOfNorm (stepAssembly)` — the norm-continuous extension of the
...
generators,
  ...
  `integral B hB T (stepIndicatorLp T i) = incrementLp B hB i`,
...
`∫ 𝟙_{(lo,hi]} dB = B_hi − B_lo`. The extension `integral` agrees with the base
...
`incrementAssembly (single i 1) = incrementLp i`. -/
lemma integral_stepIndicator (hB : IsPreBrownianReal B μ) (T : ℝ≥0) (i : StepIndex T) :
    integral B hB T (stepIndicatorLp T i) = incrementLp B hB i := by
  ...
  have hincr : incrementLp B hB i = incrementAssembly B hB T (Finsupp.single i (1 : ℝ)) := by
    rw [incrementAssembly, Finsupp.linearCombination_single, one_smul]
  rw [hstep, hincr, integral]
  exact LinearMap.extendOfNorm_eq (stepAssembly_denseRange T)
    ⟨1, fun z ↦ by rw [one_mul]; exact (incrementAssembly_isometry hB T z).le⟩ _
