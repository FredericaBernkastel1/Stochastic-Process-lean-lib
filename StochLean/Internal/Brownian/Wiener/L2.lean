...
MathFin/Foundations/WienerIntegralL2.lean (Apache-2.0).
The proof is internalized here under StochLean ownership and has no external package dependency. -/

public import StochLean.Internal.Brownian.Wiener.Kernel
...
Builds the continuous linear isometry
  ...
  `integral : Lp ℝ 2 (volume.restrict (Set.Ioc 0 T)) →L[ℝ] Lp ℝ 2 μ`
...
2. Two linear maps out of the finitely supported coefficient space
   ...
   * `incrementAssembly`: `δ_(s, t) ↦ [fun ω ↦ B t ω - B s ω]` in `Lp ℝ 2 μ`.
3. `‖incrementAssembly f‖ = ‖stepAssembly f‖` for every `f : StepIndex T →₀ ℝ`,
...
5. `LinearMap.extendOfNorm` yields the CLM `integral`, an isometry.

## Main results

* `incrementAssembly_isometry`: the step-function Itô isometry on
  `StepIndex T →₀ ℝ`.
* `stepAssembly_denseRange`: step indicators span a dense subspace of
  `Lp ℝ 2 (volume.restrict (Set.Ioc 0 T))`.
* `integral`: the Wiener integral as a `ContinuousLinearMap` from
  `Lp ℝ 2 (volume.restrict (Set.Ioc 0 T))` to `Lp ℝ 2 μ`, via
  `LinearMap.extendOfNorm`.
* `norm_integral`: the Itô isometry `‖integral f‖ = ‖f‖`.
* `integral_sq`: the Itô isometry in integral form,
  `∫ ω, (I f ω)² ∂μ = ∫ s in (0, T], (f s)² ∂volume`.
-/

@[expose] public section

-- The audited source uses `show` for definitional transport and proposition-valued local
-- instances in density arguments; retain those proof terms without style rewrites.
...
noncomputable def incrementLp (B : ℝ≥0 → Ω → ℝ)
...
noncomputable def incrementAssembly (B : ℝ≥0 → Ω → ℝ)
    (hB : IsPreBrownianReal B μ) (T : ℝ≥0) :
    (StepIndex T →₀ ℝ) →ₗ[ℝ] Lp ℝ 2 μ :=
  Finsupp.linearCombination ℝ (incrementLp B hB (T := T))
...
private lemma covariance_increment_arithmetic
  ...
  push_cast
  have hs0 : 0 ≤ (s : ℝ) := s.coe_nonneg
  have ht0 : 0 ≤ (t : ℝ) := t.coe_nonneg
  have hu0 : 0 ≤ (u : ℝ) := u.coe_nonneg
  have hv0 : 0 ≤ (v : ℝ) := v.coe_nonneg
...
private lemma inner_incrementLp_eq (hB : IsPreBrownianReal B μ)
    {T : ℝ≥0} (i j : StepIndex T) :
    ⟪incrementLp B hB i, incrementLp B hB j⟫_ℝ =
  ...
  have hLHS : ⟪incrementLp B hB i, incrementLp B hB j⟫_ℝ =
              max 0 ((min (i.hi : ℝ) j.hi) - (max (i.lo : ℝ) j.lo)) := by
    rw [L2.inner_def]
    have h_eq : ∀ᵐ ω ∂μ,
        (⟪(incrementLp B hB i : Ω → ℝ) ω,
          (incrementLp B hB j : Ω → ℝ) ω⟫_ℝ : ℝ) =
      ...
      filter_upwards [MemLp.coeFn_toLp (memLp_increment_two hB i),
        ...
        with ω hωI hωJ
      rw [show (incrementLp B hB i : Ω → ℝ) ω = B i.1.2 ω - B i.1.1 ω from hωI,
          show (incrementLp B hB j : Ω → ℝ) ω = B j.1.2 ω - B j.1.1 ω from hωJ]
...
theorem incrementAssembly_isometry (hB : IsPreBrownianReal B μ) (T : ℝ≥0)
    (f : StepIndex T →₀ ℝ) :
    ‖incrementAssembly B hB T f‖ = ‖stepAssembly T f‖ := by
  -- Squares of both norms equal a common double-sum over `f.support × f.support`.
  have h_sq : ‖incrementAssembly B hB T f‖ ^ 2 = ‖stepAssembly T f‖ ^ 2 := by
    rw [← @real_inner_self_eq_norm_sq _ _ _ (incrementAssembly B hB T f),
        ← @real_inner_self_eq_norm_sq _ _ _ (stepAssembly T f)]
    simp only [incrementAssembly, stepAssembly, Finsupp.linearCombination_apply]
...
noncomputable def integral (B : ℝ≥0 → Ω → ℝ)
    (hB : IsPreBrownianReal B μ) (T : ℝ≥0) :
    Lp ℝ 2 (volume.restrict (Set.Ioc (0 : ℝ) (T : ℝ))) →L[ℝ] Lp ℝ 2 μ :=
  (incrementAssembly B hB T).extendOfNorm (stepAssembly T)

/-- Itô isometry, norm form. For every `f ∈ L²([0, T])`,
`‖integral f‖ = ‖f‖`. -/
theorem norm_integral (hB : IsPreBrownianReal B μ) (T : ℝ≥0)
    (f : Lp ℝ 2 (volume.restrict (Set.Ioc (0 : ℝ) (T : ℝ)))) :
    ‖integral B hB T f‖ = ‖f‖ := by
  rw [integral]
  exact LinearMap.norm_extendOfNorm_eq_of_isometry
    (stepAssembly_denseRange T) (incrementAssembly_isometry hB T) f
...
`∫ ω, (integral f ω)² ∂μ = ∫ s in (0, T], (f s)² ∂volume`. -/
theorem integral_sq (hB : IsPreBrownianReal B μ) (T : ℝ≥0)
    (f : Lp ℝ 2 (volume.restrict (Set.Ioc (0 : ℝ) (T : ℝ)))) :
    ∫ ω, (integral B hB T f ω) ^ 2 ∂μ =
      ∫ s in Set.Ioc (0 : ℝ) (T : ℝ), (f s) ^ 2 ∂volume := by
  rw [← Lp_real_two_norm_sq μ (integral B hB T f),
      norm_integral hB T f,
