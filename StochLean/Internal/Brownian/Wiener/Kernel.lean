...
MathFin/Foundations/WienerIntegral.lean (Apache-2.0).
...
public import StochLean.Internal.Brownian.Wiener.ExtendOfNormIsometry
...
the canonical `ProbabilityTheory.IsPreBrownianReal` predicate):

* `step_isometry`: for a single step `c · 𝟙_{(s, t]}`,
  `∫ ω, (c · (B_t ω − B_s ω))² ∂μ = c² · (t − s)`.
* `finset_isometry`: for a monotone partition `p : Fin (n+1) → ℝ≥0`
...
the StochLean deterministic-Wiener `L²` module.
-/
...
theorem step_isometry (c : ℝ) {s t : ℝ≥0} (hst : s ≤ t) :
