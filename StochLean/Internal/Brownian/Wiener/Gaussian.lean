...
/- Adapted into StochLean from raphaelrrcoelho/formal-mathfin at commit
784a8311f75a1519a23717856df9982bd6a9a370, source
MathFin/Foundations/WienerIntegralGaussian.lean (Apache-2.0).
The proof is internalized here with no external package dependency. -/

public import StochLean.Internal.Brownian.Wiener.L2

/-!
# The Wiener integral of a deterministic integrand is Gaussian

The Wiener integral `integral B hB T : L²([0,T]) →L[ℝ] L²(μ)`
(`Foundations/WienerIntegralL2.lean`) was built as an isometry — its *norm* is
pinned (`norm_integral`, `integral_sq`) but its *law*
was not. This file proves the missing distributional fact:

  for a **deterministic** integrand `f ∈ L²([0,T])`, the random variable
  `ω ↦ (integral B hB T f) ω` is **Gaussian**, centred, with variance
  `‖f‖²₂ = ∫₀ᵀ f²`:

  `μ.map (integral B hB T f) = gaussianReal 0 ‖f‖²`.
...
`incrementAssembly` on simple (step) processes, along the dense embedding
`stepAssembly` of step indicators in `L²([0,T])` (`stepAssembly_denseRange`).

1. **Simple processes are Gaussian.** `incrementAssembly B hB T x =
   ...
   centred) and variance `‖incrementAssembly x‖² = ‖stepAssembly x‖²` (the Itô
   isometry, `incrementAssembly_isometry`).

2. **Pass to the L²-limit by characteristic functions.** For each frequency
   `t`, the map `g ↦ charFun (μ.map (integral g)) t` is continuous
...
* `integral_map_eq_gaussianReal`: `μ.map (integral B hB T f) =
  gaussianReal 0 (‖f‖²).toNNReal`.
* `integral_hasLaw_gaussian`: the `HasLaw` form with the variance in
...
elementary integral `incrementAssembly` (this is `LinearMap.extendOfNorm_eq`). -/
lemma integral_stepAssembly (hB : IsPreBrownianReal B μ) (T : ℝ≥0)
    (x : StepIndex T →₀ ℝ) :
    integral B hB T (stepAssembly T x) = incrementAssembly B hB T x := by
  rw [integral,
    LinearMap.extendOfNorm_eq (stepAssembly_denseRange T)
      ⟨1, fun y ↦ by rw [one_mul]; exact (incrementAssembly_isometry hB T y).le⟩]

/-! ### Simple processes are Gaussian -/

omit [IsProbabilityMeasure μ] in
/-- Explicit a.e. form of the elementary integral: `incrementAssembly B hB T x`
agrees a.e. with the finite sum of scaled Brownian increments. -/
lemma incrementAssembly_coeFn (hB : IsPreBrownianReal B μ) (T : ℝ≥0)
    (x : StepIndex T →₀ ℝ) :
    (incrementAssembly B hB T x : Ω → ℝ) =ᵐ[μ]
      fun ω ↦ ∑ i ∈ x.support, x i • (B i.1.2 ω - B i.1.1 ω) := by
  have hExpand : incrementAssembly B hB T x
      = ∑ i ∈ x.support, x i • incrementLp B hB i := by
    rw [incrementAssembly, Finsupp.linearCombination_apply, Finsupp.sum]
  have hterm : ∀ i ∈ x.support,
      (↑(x i • incrementLp B hB i) : Ω → ℝ)
        =ᵐ[μ] fun ω ↦ x i • (B i.1.2 ω - B i.1.1 ω) := by
    intro i _
    filter_upwards [Lp.coeFn_smul (x i) (incrementLp B hB i),
      MemLp.coeFn_toLp (memLp_increment_two hB i)] with ω hsmul htoLp
    rw [hsmul]
    simp only [Pi.smul_apply]
    rw [show (incrementLp B hB i : Ω → ℝ) ω = B i.1.2 ω - B i.1.1 ω from htoLp]
  rw [hExpand]
  refine (Lp.coeFn_finsetSum x.support (fun i ↦ x i • incrementLp B hB i)).trans ?_
...
lemma incrementAssembly_hasGaussianLaw (hB : IsPreBrownianReal B μ) (T : ℝ≥0)
    (x : StepIndex T →₀ ℝ) :
    HasGaussianLaw (fun ω ↦ (incrementAssembly B hB T x) ω) μ := by
  ...
  exact hsum.congr (incrementAssembly_coeFn hB T x).symm

/-- The elementary integral is centred: `∫ incrementAssembly B hB T x ∂μ = 0`. -/
lemma incrementAssembly_integral_zero (hB : IsPreBrownianReal B μ) (T : ℝ≥0)
    (x : StepIndex T →₀ ℝ) :
    ∫ ω, (incrementAssembly B hB T x) ω ∂μ = 0 := by
  rw [integral_congr_ae (incrementAssembly_coeFn hB T x)]
...
lemma incrementAssembly_map_eq_gaussianReal (hB : IsPreBrownianReal B μ) (T : ℝ≥0)
    (x : StepIndex T →₀ ℝ) :
    μ.map (fun ω ↦ (incrementAssembly B hB T x) ω)
      = gaussianReal 0 (‖stepAssembly T x‖ ^ 2).toNNReal := by
  rw [(incrementAssembly_hasGaussianLaw hB T x).map_eq_gaussianReal]
  have hmean : μ[fun ω ↦ (incrementAssembly B hB T x) ω] = 0 :=
    incrementAssembly_integral_zero hB T x
  have hvar : Var[fun ω ↦ (incrementAssembly B hB T x) ω; μ] = ‖stepAssembly T x‖ ^ 2 := by
    rw [variance_eq_integral
      ((Lp.aestronglyMeasurable (incrementAssembly B hB T x)).aemeasurable), hmean]
    simp only [sub_zero]
    rw [← Lp_real_two_norm_sq μ (incrementAssembly B hB T x), ← incrementAssembly_isometry hB T x]
  rw [hmean, hvar]
...
lemma L1_norm_le_L2_norm_prob (g : Lp ℝ 2 μ) :
  ...
  rw [show (∫ ω, ‖g ω‖ ∂μ) = (eLpNorm (g : Ω → ℝ) 1 μ).toReal by
    rw [eLpNorm_one_eq_lintegral_enorm,
      ← integral_norm_eq_lintegral_enorm (Lp.aestronglyMeasurable g)],
    Lp.norm_def]
...
`f ∈ L²([0,T])`, `μ.map (integral B hB T f) = gaussianReal 0 ‖f‖²`. -/
theorem integral_map_eq_gaussianReal (hB : IsPreBrownianReal B μ) (T : ℝ≥0)
    (f : Lp ℝ 2 (volume.restrict (Set.Ioc (0 : ℝ) (T : ℝ)))) :
    μ.map (fun ω ↦ (integral B hB T f) ω)
      = gaussianReal 0 (‖f‖ ^ 2).toNNReal := by
  refine Measure.ext_of_charFun (funext fun t ↦ ?_)
  have hcont1 : Continuous (fun g : Lp ℝ 2 (volume.restrict (Set.Ioc (0 : ℝ) (T : ℝ))) ↦
      charFun (μ.map (fun ω ↦ (integral B hB T g) ω)) t) := by
    ...
    exact hL.continuous.comp (integral B hB T).continuous
  ...
  have hbase : ∀ x : StepIndex T →₀ ℝ,
      charFun (μ.map (fun ω ↦ (integral B hB T (stepAssembly T x)) ω)) t
        = cexp (-(((‖stepAssembly T x‖ ^ 2 : ℝ) : ℂ) * (t : ℂ) ^ 2 / 2)) := by
    intro x
    rw [integral_stepAssembly hB T x, incrementAssembly_map_eq_gaussianReal hB T x,
      charFun_gaussianReal]
    congr 1
    rw [show (((‖stepAssembly T x‖ ^ 2).toNNReal : ℝ)) = ‖stepAssembly T x‖ ^ 2 from
      Real.coe_toNNReal _ (by positivity)]
    push_cast; ring
  have hf_eq : charFun (μ.map (fun ω ↦ (integral B hB T f) ω)) t
      = cexp (-(((‖f‖ ^ 2 : ℝ) : ℂ) * (t : ℂ) ^ 2 / 2)) :=
    (stepAssembly_denseRange T).induction_on
      (p := fun g ↦ charFun (μ.map (fun ω ↦ (integral B hB T g) ω)) t
