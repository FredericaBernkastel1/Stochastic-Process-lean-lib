...
* `tailFamily_antitone`, `tailProcess_le_tailFamily`: basic monotonicity
* `tailProcess_eq_iInf_revFiltration`: bridge to a reverse-filtration formulation
  (parameterised by an arbitrary `revFiltration`)
* `tailProcess_le_ambient`: the tail σ-algebra is a sub-σ-algebra of the ambient one
...
namespace StochLean.Internal.Exchangeability.Tail
...
end StochLean.Internal.Exchangeability.Tail
