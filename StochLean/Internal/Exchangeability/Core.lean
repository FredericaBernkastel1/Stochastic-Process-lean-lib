...
import StochLean.Internal.Exchangeability.Contractability

/-!
# StochLean.Internal.Exchangeability and Full StochLean.Internal.Exchangeability
...
-/
private def prefixCylinder {n : ℕ} (S : Set (Fin n → α)) : Set (ℕ → α) :=
  (prefixProj (α:=α) n) ⁻¹' S

omit [MeasurableSpace α] in
@[simp]
private lemma mem_prefixCylinder {n : ℕ} {S : Set (Fin n → α)} {x : ℕ → α} :
...
-/
private def prefixCylinders : Set (Set (ℕ → α)) :=
  {A | ∃ n, ∃ S : Set (Fin n → α), MeasurableSet S ∧ A = prefixCylinder (α:=α) S}

private lemma prefixCylinder_mem_prefixCylinders {n : ℕ} {S : Set (Fin n → α)}
    (hS : MeasurableSet S) :
    prefixCylinder (α:=α) S ∈ prefixCylinders (α:=α) :=
  ⟨n, S, hS, rfl⟩

private lemma measurable_of_mem_prefixCylinders {A : Set (ℕ → α)}
...
-/
private def takePrefix (hmn : m ≤ n) (x : Fin n → α) : Fin m → α :=
  fun i => x (Fin.castLE hmn i)

omit [MeasurableSpace α] in
@[simp]
private lemma takePrefix_prefixProj {hmn : m ≤ n} (x : ℕ → α) :
...
-/
private def extendSet (hmn : m ≤ n) (S : Set (Fin m → α)) : Set (Fin n → α) :=
  {x | takePrefix (α:=α) hmn x ∈ S}

omit [MeasurableSpace α] in
private lemma prefixCylinder_inter {m n : ℕ} {S : Set (Fin m → α)} {T : Set (Fin n → α)} :
...
@[measurability]
private lemma extendSet_measurable {m n : ℕ} {S : Set (Fin m → α)} {hmn : m ≤ n}
...
-/
private lemma isPiSystem_prefixCylinders :
...
private lemma cylinder_subset_prefixCylinders {s : Finset ℕ} {S : Set (∀ _ : s, α)}
    (hS : MeasurableSet S) :
  ...
  have h_mem : ∀ i ∈ s, i < N := by
    intro i hi
    have hle : i ≤ s.sup id := Finset.le_sup (f := @id ℕ) hi
    omega
  ...
  have hpull_meas : Measurable pull := by
    ...
    intro y
    exact measurable_pi_apply (⟨y.1, h_mem y.1 y.2⟩ : Fin N)
...
-/
private lemma generateFrom_prefixCylinders :
...
/-!
## StochLean.Internal.Exchangeability versus full exchangeability
...
StochLean.Internal.Exchangeability ensures the finite marginals match, and by the previous
...
private lemma pathLaw_map_prefix (μ : Measure Ω) (X : ℕ → Ω → α)
...
private lemma pathLaw_map_prefix_perm (μ : Measure Ω) (X : ℕ → Ω → α)
...
-/
private def permBound : ℕ :=
  max n ((Finset.range n).sup fun i : ℕ => π i + 1)

private lemma le_permBound : n ≤ permBound π n := le_max_left _ _

private lemma lt_permBound_of_lt {i : ℕ} (hi : i < n) :
...
private lemma lt_permBound_fin {i : Fin n} :
...
-/
private def approxPerm : Equiv.Perm (Fin (permBound π n)) :=
  Classical.choose (exists_approxPerm (π:=π) (n:=n))

private lemma approxPerm_apply_cast {i : Fin n} :
    approxPerm (π:=π) (n:=n)
...
@[simp]
private lemma approxPerm_apply_cast_coe {i : Fin n} :
    ((approxPerm (π:=π) (n:=n)
...
/--
StochLean.Internal.Exchangeability implies invariance of all finite marginals under arbitrary permutations.
...
`approxPerm`. StochLean.Internal.Exchangeability ensures this finite permutation preserves the
n-dimensional marginal, hence so does π.
-/
private lemma marginals_perm_eq {μ : Measure Ω} (X : ℕ → Ω → α)
...
/--
**Main theorem:** StochLean.Internal.Exchangeability and full exchangeability are equivalent.
