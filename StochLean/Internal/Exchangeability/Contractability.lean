...
import StochLean.Internal.Exchangeability.Probability.InfiniteProduct
import StochLean.Internal.Exchangeability.Util.StrictMono

/-!
# Contractability and StochLean.Internal.Exchangeability
...
- **Contractable → exchangeable** (`StochLean.Internal.Exchangeability/DeFinetti/Theorem.lean`): Uses ergodic
  theory and the martingale convergence approach.
- **Exchangeable ↔ fully exchangeable** (`StochLean.Internal.Exchangeability.Core`):
  Uses π-system uniqueness and finite approximation of infinite permutations.
- **Conditionally i.i.d. → exchangeable** (`StochLean.Internal.Exchangeability/ConditionallyIID.lean`):
...
permutations of ℕ, not just finite ones. However, `StochLean.Internal.Exchangeability.Core` proves
...
/-- StochLean.Internal.Exchangeability at a specific dimension n. -/
...
/-- StochLean.Internal.Exchangeability is preserved under composition of permutations. -/
  ...
  | succ m' =>
    ...
    have hlhs_eq : (proj ∘ fun ω j => X (σ j).val ω) = (fun ω i => X (k i) ω) := by
      ...
      have hσi : (σ (ι i)).val = k i := by
        change (σ (Fin.castLE hmn i)).val = k i
        exact hσ i
