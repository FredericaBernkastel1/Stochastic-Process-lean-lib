...
private lemma timeReversal_crossing_bound_strong
  ...
  | succ m' ih =>
    ...
    have hX_τ_le_a : X τ ω ≤ a := by
      have h_neq_τ' : lowerCrossingTime a b X N (j - 1) ω ≠ N := by
        simpa only [hτ_def] using h_neq_τ
      have h := stoppedValue_lowerCrossingTime (f := X) (n := j - 1) h_neq_τ'
      simp only [stoppedValue, ← hτ_def] at h
      exact h
