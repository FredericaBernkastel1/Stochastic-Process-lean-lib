...
namespace StochLean.Internal.Exchangeability.Probability.IntegrationHelpers
...
lemma abs_integral_mul_le_L2
  ...
  have hf_abs : MemLp (fun ω => |f ω|) (ENNReal.ofReal 2) μ := by
    have h := hf.abs
    change MemLp (fun ω => |f ω|) 2 μ at h
    convert h using 1 <;> norm_num
  have hg_abs : MemLp (fun ω => |g ω|) (ENNReal.ofReal 2) μ := by
    have h := hg.abs
    change MemLp (fun ω => |g ω|) 2 μ at h
    convert h using 1 <;> norm_num
...
lemma L2_tendsto_implies_L1_tendsto_of_bounded
  ...
  have hbound : ∀ n, ∫ ω, |f n ω - g ω| ∂μ ≤ (∫ ω, (f n ω - g ω)^2 ∂μ) ^ (1/2 : ℝ) := by
    ...
    have h_abs_memLp : MemLp (fun ω => |f n ω - g ω|) 2 μ := by
      have h := h_memLp.abs
      change MemLp (fun ω => |f n ω - g ω|) 2 μ at h
      exact h
...
end StochLean.Internal.Exchangeability.Probability.IntegrationHelpers
