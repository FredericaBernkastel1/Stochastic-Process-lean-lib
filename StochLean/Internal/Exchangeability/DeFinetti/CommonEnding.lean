...
See also `StochLean.Internal.Exchangeability.ConditionallyIID` for the definition of conditionally i.i.d. sequences
...
namespace StochLean.Internal.Exchangeability.DeFinetti.CommonEnding

open MeasureTheory ProbabilityTheory
open StochLean.Internal.Exchangeability.PathSpace (shift shift_measurable IsShiftInvariant isShiftInvariant_iff)
open scoped BigOperators
open Set
open StochLean.Internal.Exchangeability
...
-- and is now defined in StochLean.Internal.Exchangeability.PathSpace.Shift to avoid duplication across the codebase.
...
private lemma measure_via_indicator_integral (μ : Measure Ω) (X : ℕ → Ω → α)
  ...
  have hEvtMeas : MeasurableSet E := by
    ...
    rw [this]
    change MeasurableSet (⋂ i : Fin m, X (k i) ⁻¹' B i)
...
Outline (to be implemented):
  ...
  • **Invoke `exchangeable_of_conditionallyIID`** (see
    `StochLean.Internal.Exchangeability/ConditionallyIID.lean`) once the `conditionallyIID` record
...
end StochLean.Internal.Exchangeability.DeFinetti.CommonEnding
