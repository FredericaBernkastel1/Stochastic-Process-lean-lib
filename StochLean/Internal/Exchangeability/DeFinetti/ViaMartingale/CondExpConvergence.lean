...
import StochLean.Internal.Exchangeability.DeFinetti.ViaMartingale.KallenbergChain
...
lemma condexp_convergence
  ...
  have h := StochLean.Internal.Exchangeability.Probability.condexp_indicator_eq_of_pair_law_eq
...
def M (hX_meas : ∀ n, Measurable (X n)) (k : ℕ) (B : Set α) (_hB : MeasurableSet B) :
    ℕ → Ω → ℝ :=
  fun m => StochLean.Internal.Exchangeability.Probability.condExpWith μ (futureFiltration X m)
