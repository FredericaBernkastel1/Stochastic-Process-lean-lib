...
open MartingaleHelpers StochLean.Internal.Exchangeability.PathSpace
...
lemma preimage_rect_future
  ...
  · rcases h with ⟨hB, hC⟩
    ...
    · simpa [ψ]
    · intro i
      change X (m + 1 + i.1) ω ∈ C i
      simpa only [ψ, shiftRV] using
        ((hC : (shiftRV X (m + 1) ω) ∈ cylinder (α:=α) r C) i)
  · rcases h with ⟨hB, hC⟩
    refine ⟨?_, ?_⟩
    · simpa [ψ]
    · intro i
